...
class _MyHomePageState extends State<MyHomePage>
  ...
  String? currentUserRole; // To store the logged-in user's role
  ...
  void initState() {
    ...
    _fetchCurrentUserRole(); // Fetch the current user's role on initialization
  }
    ...
    if (currentUser != null) {
      try {
        ...
        if (userDoc.exists) {
          ...
          setState(() {
            currentUserRole = "unknown"; // Fallback
          });
        }
      } catch (e) {
        setState(() {
          currentUserRole = "error"; // Handle error case
        });
      }
    }
  }
                ...
                ProductBox(
                  ...
                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => const FileChatScreen()),
                    );
                  },
                ),
              const SizedBox(height: 10),
              Expanded(
                child: currentUserRole == null
                    ? const Center(
                        child:
                            CircularProgressIndicator()) // Show loading until role is fetched
                    : StreamBuilder<QuerySnapshot>(
                        stream: FirebaseFirestore.instance
                            .collection('users')
                            .where('role',
                                isEqualTo: currentUserRole == 'patient'
                                    ? 'doctor'
                                    : 'patient')
                            .snapshots(),
